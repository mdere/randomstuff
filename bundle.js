...
/***/ function(module, exports, __webpack_require__) {
    ...
    module.exports = __webpack_require__(/*! /opt/app/client/js/app.js */298);
...
/***/ function(module, exports, __webpack_require__) {
    ...
    __webpack_require__(/*! @orange-games/phaser-input/build/phaser-input */ 305);
    __webpack_require__(/*! ./Main */ 306);

    game.state.start('Boot');

    function bootState() {
      return game.state.states.Boot;
    }

    function preLoaderState() {
      return game.state.states.Preloader;
    }

    function mainMenuState() {
      return game.state.states.MainMenu;
    }

    function gameWorldState() {
      return game.state.states.GameWorld;
    }
...
/* 305 */
/*!************************************************************!*\
  !*** ./~/@orange-games/phaser-input/build/phaser-input.js ***!
  \************************************************************/
/***/ function(module, exports) {
...
/***/ function(module, exports, __webpack_require__) {
    ...
    window.game = new Phaser.Game(800, 600, Phaser.AUTO, 'game-world');

    game.state.add('Boot',      __webpack_require__(/*! ./game/states/Boot */ 307));
    game.state.add('Preloader', __webpack_require__(/*! ./game/states/Preloader */ 308));
    game.state.add('MainMenu',  __webpack_require__(/*! ./game/states/MainMenu */ 309));
    game.state.add('GameWorld', __webpack_require__(/*! ./game/states/GameWorld */ 310));


/***/ },
/* 307 */
/*!***************************************!*\
  !*** ./client/js/game/states/Boot.js ***!
  \***************************************/
/***/ function(module, exports) {
    ...
    Boot.prototype = {
      ...
      preload:function() {
        console.log("PRELOADING BOOT");
        game.load.image('preloaderBar', '/client/assets/preloader.png');
      },
      ...
      //   this.bootBar = this.add.sprite(this.game.world.centerX, this.game.world.centerY + 200, 'boot_bar')
      //   this.bootBar.anchor(0.5);
      //   this.load.setPreloadSprite(this.bootBar);
      // }
      
      // create: function() {
      //   this.game.stage.background = 
      // }
...
/* 308 */
/*!********************************************!*\
  !*** ./client/js/game/states/Preloader.js ***!
  \********************************************/
/***/ function(module, exports) {
    ...
    Preloader.prototype = {
      ...
      displayLoadScreen: function() {
        console.log("DISPLAYLOAD SCREEN PRELOADER");
        this.onLoadComplete();
      },
      ...
      loadAssets: function() {
        ...
        game.load.atlasJSONHash(
          'ship-sprites',
          '/client/assets/ship-sprites.png',
          '/client/assets/ship-sprites.json'
        );
        game.load.atlasJSONHash(
          'asteroid-sprites',
          '/client/assets/asteroid-sprite.png',
          '/client/assets/asteroid-sprite.json'
        );    
        console.log("LOADASSETS PRELOADER");
      },
...
/* 309 */
/*!*******************************************!*\
  !*** ./client/js/game/states/MainMenu.js ***!
  \*******************************************/
/***/ function(module, exports) {
    ...
    MainMenu.prototype = {
      preload: function() {
        console.log("PRELOADING MAINMENU");
        game.load.image('loginButton', '/client/assets/MainMenu/Login.png')
      },
      
      create: function() {
        console.log("LOADING GameWorld");

        this.userNameField = game.add.inputField(10, 90);

        this.loginButton = game.add.button(game.world.centerX, 400, 'loginButton', this.login, this);
        this.loginButton.scale.setTo(2);

      },

      update: function() {
        if (this.session) {
          game.state.start('GameWorld');
          console.log("LOGGED IN");
        }
      },

      // This will hit API Service with credentials
      login: function() {
        console.log(this);
        this.session = true;
...
/* 310 */
/*!********************************************!*\
  !*** ./client/js/game/states/GameWorld.js ***!
  \********************************************/
/***/ function(module, exports, __webpack_require__) {

    var Ship = __webpack_require__(/*! ../entities/ship */ 311);
    var AsteroidGroupEntity = __webpack_require__(/*! ../entities/AsteroidGroup */ 312);
    ...
    var GameWorld = function() {
      ...
      this.target_asteroid = null;
      this.asteroid_counter = 1;
      
      this.player1 = null;
      
      this.targetable_asteroids = [];
      this.continous_mining = false;
      
      this.tilesprite = null;
      
      this.max_objects = 200;
    };
    ...
    GameWorld.prototype = {
      create: function() {
        ...
        this.tilesprite = game.add.tileSprite(0, 0, 5000, 5000, 'background');
        
        this.asteroid_group = game.add.physicsGroup();
        this.asteroidGroupEntity = new AsteroidGroupEntity(this.asteroid_group);

        // Asteroids
        for (var i = 0; i < this.max_objects; i++) {
          var loc_finding = true;
          while (loc_finding) {
            var loc_x = this.rnd.between(900, 5000);
            var loc_y = this.rnd.between(900, 5000);
            var primary_composite = this.ore_array[Math.floor(Math.random() * this.ore_array.length)];
            var secondary_composite = this.ore_array[Math.floor(Math.random() * this.ore_array.length)];
            var asteroid = this.asteroidGroupEntity.create(this.asteroid_counter, loc_x, loc_y,
              {
                durability: Math.floor(Math.random() * 10),
                primary_composite: primary_composite,
                primary_tint: this.ore_color[primary_composite],
                secondary_composite: secondary_composite,
                alpha: [true, false][Math.floor(Math.random() * 1)],
                alpha_composite: this.ore_array[Math.floor(Math.random() * this.ore_array.length)]
              }
            );
            if (!this.isAsteroidColliding(asteroid, this.asteroids)) {
              this.asteroid_counter += 1;
              loc_finding = false;
            } else if (this.asteroid_group.hash.length > this.max_objects) {
              loc_finding = false;
            } else {
              this.asteroid_group.remove(asteroid);
            }
          }
        }

        // Ship
        this.player1 = Ship.create(game);
        
        // Asteroid keyboard + logic
        this.player1.target_asteroid.onDown.add(
          function() {
            this.targetAsteroid();
          }, this
        );
        
        this.player1.mine.onDown.add(
          function() {
            this.mineAsteroid();
          }, this
        );
        
        this.player1.continuous_mining.onDown.add(
          function() {
            if (this.continous_mining) {
              console.log("Continous Mining set to false");
              this.continous_mining = false;
            } else {
              console.log("Continous Mining set to true");
              this.continous_mining = true;
            }
          }, this
        )
      },
      ...
      update: function() {
        Ship.movement(game, this.player1);
          
        if (this.continous_mining) {
          if (this.target_asteroid) {
            this.mineAsteroid();
          } else {
            this.targetAsteroid();
          }
        ...
        if (this.physics.arcade.collide(this.player1, this.asteroid_group, this.collisionHandler, this.processHandler, this)) {
        // console.log("SHIP HIT ASTEROID!");
        } else if (this.physics.arcade.collide(this.asteroid_group)) {
        // console.log("ASTEROID HIT ASTEROID!");
        }
        if (this.physics.arcade.collide(this.player1.triggerBox, this.asteroid_group)) {
          // console.log("INTERSECTED!");
        }
      },
      
      render: function() {
        // this.game.debug.text("Session: "+ sessionId, this.game.width - 200, 15);
      
        game.debug.spriteInfo(this.player1, 32, 32);
        game.debug.text('Controls:', game.width - 200, game.height - 100);
        game.debug.text('Accelerate (W)', game.width - 200, game.height - 85);
        game.debug.text('Pitch Left (A)', game.width - 200, game.height - 70);
        game.debug.text('Pitch Right (D)', game.width - 200, game.height - 55);
        game.debug.text('Target Asteroid (R)', game.width - 200, game.height - 40);
        game.debug.text('Mine Asteroid (M)', game.width - 200, game.height - 25);
        game.debug.text('Continous Mining (T)', game.width -200, game.height - 10)

        game.debug.text("Rock", 50, game.height - 60);
        game.debug.text(this.player1.inventory.rock, 50, game.height - 30);
        game.debug.text("Fe", 100, game.height - 60);
        game.debug.text(this.player1.inventory.iron, 100, game.height - 30);
        game.debug.text("Ni", 150, game.height - 60);
        game.debug.text(this.player1.inventory.nickel, 150, game.height - 30);
        game.debug.text("Co", 200, game.height - 60);
        game.debug.text(this.player1.inventory.cobalt, 200, game.height - 30);
        game.debug.text("Mg", 250, game.height - 60);
        game.debug.text(this.player1.inventory.magnesium, 250, game.height - 30);
        game.debug.text("Si", 300, game.height - 60);
        game.debug.text(this.player1.inventory.silicon, 300, game.height - 30);
        game.debug.text("Au", 350, game.height - 60);
        game.debug.text(this.player1.inventory.gold, 350, game.height - 30);
        game.debug.text("Pt", 400, game.height - 60);
        game.debug.text(this.player1.inventory.platinum, 400, game.height - 30);
        game.debug.text("U", 450, game.height - 60);
        game.debug.text(this.player1.inventory.uranium, 450, game.height - 30);
      },

      // Decoupled logic methods, this needs to be refactored so it can be within the
      // game object scope.

      targetAsteroid: function() {
        for (var index in this.asteroid_group.hash) {
          var current_asteroid = this.asteroid_group.hash[index];
          // Check if within distance of astreroid
          var is_within_distance = this.isAsteroidWithinDistance(this.player1, current_asteroid);
          var active_asteroid_index = this.targetable_asteroids.indexOf(current_asteroid);
          if (active_asteroid_index > -1) {
            if (!is_within_distance) {
              this.targetable_asteroids.splice(active_asteroid_index, 1);
            } 
          } else if (active_asteroid_index < 0 && is_within_distance) {
            this.targetable_asteroids.push(current_asteroid);
          }
        }
        if (this.targetable_asteroids.length > 0) {
          if (this.target_asteroid) {
            this.target_asteroid.tint = this.target_asteroid.primary_tint;
          }
          var index_of_targetable_asteroid = this.targetable_asteroids.indexOf(this.target_asteroid);
          if (this.target_asteroid && index_of_targetable_asteroid > -1) {
            this.target_asteroid = this.nextTargetableAsteroid(index_of_targetable_asteroid);
            if (this.target_asteroid) {
              this.target_asteroid.tint = '0x96ff7f';
            }
          } else {
            this.target_asteroid = this.targetable_asteroids[0];
            this.target_asteroid.tint = '0x96ff7f';
          }
        }
        if (this.target_asteroid) {
          console.log("Targeted asteroid", this.target_asteroid);
        }
      },
      
      mineAsteroid: function() {
        if ((game.time.now - this.player1.last_action) >= this.player1.mining_speed && this.target_asteroid) {
          var is_within_distance = this.isAsteroidWithinDistance(this.player1, this.target_asteroid);
          if (is_within_distance) {
            this.emitAsteroidDebris(this.target_asteroid);
            if (this.target_asteroid.durability > 1) {
              this.target_asteroid.durability -= 1;
              this.awardShip(this.target_asteroid, 1);
            } else {
              this.awardShip(this.target_asteroid, 1);
              this.killAsteroid(this.target_asteroid);
            }
          } else {
            this.targetAsteroid();
          }
          this.player1.last_action = this.time.now;
        } else {
          // console.log("Ship not ready to mine!")
        } 
      },
      
      emitAsteroidDebris: function(asteroid) {
        var asteroid_debris_emitter = this.add.emitter(asteroid.x, asteroid.y, 20);
        asteroid_debris_emitter.makeParticles('asteroid-sprites', 'sprite_asteroid_00.png');
        asteroid_debris_emitter.setXSpeed(-100, 100);
        asteroid_debris_emitter.setYSpeed(-100, 100);
        asteroid_debris_emitter.forEach(function(particle) {
          particle.body.allowGravity = false;
        });
        asteroid_debris_emitter.setScale(0.05, 0.1, 0.05, 0.1);
        asteroid_debris_emitter.explode(4000, Math.floor(Math.random()*20) + 1);
      },
      
      isAsteroidWithinDistance: function(ship, asteroid) {
        if (asteroid) {
          if (game.physics.arcade.distanceBetween(ship, asteroid) < 50 + asteroid.width) {
            return true;
          } else {
            return false;
          }
      ...
      isAsteroidColliding: function(asteroid) {
        if (game.physics.arcade.overlap(asteroid, this.asteroid_group)) {
          return true;
        } else {
          return false
        }
      ...
      nextTargetableAsteroid: function(id) {
        var total_length = this.asteroid_group.length;
        if (id >= total_length - 1) {
          return this.asteroid_group[0];
        } else if (id < total_length -1) {
          return this.asteroid_group[id + 1];
        } else {
          return this.asteroid_group[id];
        }
      },
      
      // This should be a method in GameWorld to Kill Asteroid
      killAsteroid: function(asteroid) {
        this.generateAsteroidChunks(asteroid)
        asteroid.kill();
        this.targetable_asteroids.splice(this.targetable_asteroids.indexOf(asteroid), 1);
        this.asteroid_group.remove(this.target_asteroid);
        this.target_asteroid = null;
      },
      
      awardShip: function(entity, amount) {
        var mineral_composition = entity.mineral_composition;
        this.player1.inventory[mineral_composition.primary_composite.type] += amount;
      },
      
      // Purpose of this method is to take mineral data and return health and chunks for asteroid
      // For example if mineral_data returns the following:
      // { rock: 100, iron: 50 }
      // We should randomize and return asteroid chunks of any size and HP of X amount of rock and iron
      // for that it's total amount equates to the sum of the group of chunks.
      // {asteroid1: { rock: 10 }, asteroid2: { rock: 25 }, asteroid3: {rock: 65} ....etc}
      generateAsteroidChunks: function(asteroid) {
        var chunks = {};
        var mineral_composition = asteroid.mineral_composition;
        if (!(asteroid.parent_id > 0)) {
          for (var composition in mineral_composition) {
            var split_counter = Math.floor(Math.random()*5 + 1);
            for (var i = 0; i < split_counter; i++) {
              if (mineral_composition[composition]["durability"] > 0) {
                var new_asteroid_durability = Math.floor(Math.random() * mineral_composition[composition]["durability"]);
                if (new_asteroid_durability > 0) {
                  var random_size = Math.round(100 * ((Math.random() * 0.65) + 0.2)) / 100;
                  var new_asteroid = this.asteroidGroupEntity.create(this.asteroid_counter, asteroid.x, asteroid.y, {
                    parent_id: asteroid.id,
                    primary_composite: mineral_composition[composition]["type"],
                    is_chunk: true,
                    set_random_velocity: true,
                    primary_tint: this.ore_color[ mineral_composition[composition]["type"]],
                    scale_x: random_size,
                    scale_y: random_size,
                    durability: new_asteroid_durability,
                  });
                  this.asteroid_counter += 1;
                }
              }
...
/* 311 */
/*!*****************************************!*\
  !*** ./client/js/game/entities/ship.js ***!
  \*****************************************/
/***/ function(module, exports) {
    ...
    var create = function (game) {
        ...
        // Enable physcis for Sprite
        game.physics.enable(ship, Phaser.Physics.ARCADE);
        game.camera.follow(ship, Phaser.Camera.FOLLOW_TOPDOWN);

        ship.body.drag.set(70);
        ship.body.maxVelocity.set(200);
        ship.body.setSize(ship.width * 0.80, ship.height * 0.80);
        ship.animations.add(
            'acceleration',
            ["player-ship01.png", "player-ship02.png", "player-ship03.png", "player-ship02.png"],
            5, true, false
        );
        ship.body.collideWorldBounds = true;

        ship.accelerate  = game.input.keyboard.addKey(Phaser.KeyCode.W);
        ship.pitch_left  = game.input.keyboard.addKey(Phaser.KeyCode.A);
        ship.pitch_right = game.input.keyboard.addKey(Phaser.KeyCode.D);
        
        ship.target_asteroid = game.input.keyboard.addKey(Phaser.KeyCode.R);
        ship.mine = game.input.keyboard.addKey(Phaser.KeyCode.M);
        ship.continuous_mining = game.input.keyboard.addKey(Phaser.KeyCode.T);
        ...
        ship.mining_speed = 2000;
        ship.last_action = 0;
        return ship;
    }

    var movement = function (game, ship) {
        if (ship.accelerate.isDown) {
            game.physics.arcade.accelerationFromRotation(ship.rotation - 1.57, 300, ship.body.acceleration);
            ship.animations.play('acceleration');
        } else {
            ship.body.acceleration.set(0);
            ship.animations.stop();
        }

        if (ship.pitch_left.isDown)
        {
            ship.body.angularVelocity = -300;
        }
        else if (ship.pitch_right.isDown)
        {
            ship.body.angularVelocity = 300;
        }
        else
...
/* 312 */
/*!**************************************************!*\
  !*** ./client/js/game/entities/AsteroidGroup.js ***!
  \**************************************************/
/***/ function(module, exports) {
    ...
    class AsteroidGroup {
      constructor(asteroid_group) {
        this.asteroid_group = asteroid_group;
      }
      
      create(id, loc_x, loc_y, options) {
        var asteroid = this.asteroid_group.create(
          loc_x,
          loc_y,
          'asteroid-sprites',
          'sprite_asteroid_00.png'
        );
        asteroid.id = this.findAsteroid(asteroid);
        // // asteroid.body.mass = 1;
        asteroid.anchor.setTo(0.5);
        asteroid.body.collideWorldBounds = true;
        asteroid.body.bounce.setTo(1,1);
        asteroid.body.minBounceVelocity = 0;
        var asteroid_animation = this.loadAsteroidAnimation();
        asteroid.animations.add(
          'asteroid-spin',
          asteroid_animation,
          5, true, false
        );
        asteroid.play('asteroid-spin');
        if (options) {
          if (options["durability"]) {
            asteroid.durability = options["durability"];
          } else {
            asteroid.durability = Math.floor(Math.random()*10) + 5;
          }
          if (options["primary_tint"]) {
            asteroid.primary_tint = options["primary_tint"];
            asteroid.tint = asteroid.primary_tint;
          } else {
            asteroid.primary_tint = null;
            asteroid.tint = "0xffffff";
          }
          
          // This block is an issue.
          if (options["scale_x"] && options["scale_y"]) {
            if ((options["scale_x"] === options["scale_y"])) {
              asteroid.scale.setTo(options["scale_x"]);
            } else {
              asteroid.scale.setTo(options["scale_x"], options["scale_y"]);
            }
          }
          if (options["set_body_width"] && options["set_body_height"]) {
            asteroid.body.setSize(options["set_body_width"], options["set_body_height"]);
          } else {
            asteroid.body.setSize(asteroid.width * 0.80, asteroid.height * 0.80);
          }
          if (options["set_random_velocity"]) {
            asteroid.body.velocity.set(game.rnd.integerInRange(-100,100), game.rnd.integerInRange(-100,100));
          }
          if (options["parent_id"]) {
            asteroid.parent_id = options["parent_id"];
          }
          asteroid.mineral_composition = this.generateAsteroidOreData(asteroid, options);
        } else {
          asteroid.primary_type = "rock"
          asteroid.secondary_type = "rock"
        }
        return asteroid;
      }
        
      loadAsteroidAnimation() {
        var asteroid_animation = []
        for (var i = 1; i <= 47; i++) {
          var value;
          if (i < 10) {
            value = "0" + i;
          ...
          asteroid_animation.push("sprite_asteroid_" + value + ".png");
        }
        return asteroid_animation;
      }

      // Primary composite is what the majority of the asteroid will be
      // Secondary composite provides up to 50% of the total composite of Asteroid
      // Alpha is used to provide more "rare resource" to the ore itself, perhaps based on RNG?.
      // Alpha is a boolean value
      generateAsteroidOreData(asteroid, data) {
        var current_composite = {};
        if (data["primary_composite"]) {
          if (!data["secondary_composite"] && !data["alpha_composite"]) {
            current_composite["primary_composite"] = {
              type: data["primary_composite"],
              durability: asteroid.durability
            }       
          } else {
            var primary_threshold = Math.round(100 * ((Math.random() * (0.75 - 0.65)) + 0.65)) / 100;
            current_composite["primary_composite"] = {
              type: data["primary_composite"],
              durability: Math.floor(asteroid.durability * primary_threshold)
            }        
          }
        }
        if (data["secondary_composite"]) {
          if (data["alpha"]) {
            var secondary_threshold = Math.round(100 * ((1 - primary_threshold) - (0.10 * Math.random()))) / 100;
          } else {
            var secondary_threshold = Math.round(100 * (1 - primary_threshold)) / 100;
          }
          current_composite["secondary_composite"] = {
            type: data["secondary_composite"],
            durability: Math.floor(asteroid.durability * secondary_threshold)
          };      
        }
        if (data["alpha"] && data["alpha_composite"]) {
          var alpha_threshold = Math.round(100 * (1 - (primary_threshold + secondary_threshold))) / 100;
          current_composite["alpha_composite"] = {
            type: data["alpha_composite"],
            durability: Math.floor(asteroid.durability * alpha_threshold)
          };
        }
        return current_composite;
      }
      
      findAsteroid(asteroid) {
        return this.asteroid_group.hash.indexOf(asteroid);
      }
